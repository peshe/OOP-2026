...
void insertStudentAt(std::fstream& file, int index, int total, const Student& newStudent) {
    if (!file) {
        std::cerr << "Error opening file.\n";
        return;
    }
...
int main() {
    ...
    int index;
    std::cout << "Индекс за вмъкване (0-" << total << "): ";
    std::cin >> index;
    ...
    Student newStudent = {"Frank", 24, 3.70f};
    insertStudentAt(file, index, total, newStudent);
    total++;

    std::cout << "Актуализиран списък:\n";
